...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, norm_layer=None,
        ...
        if self.liu:
            ...
            self.classifier2_3 = nn.Linear(128, 2)
        elif self.crossCBAM:
            ...
            self.classifier1 = nn.Linear(1024, 5)
            self.classifier2 = nn.Linear(1024, self.num_classes)
            self.classifier_specific_1 = nn.Linear(1024, 5)
            ...
            self.branch_bam1 = CBAM(512 * block.expansion)
            self.branch_bam2 = CBAM(512 * block.expansion)
            self.classifier_specific_1 = nn.Linear(512 * block.expansion, 5)
            ...
            self.classifier1 = nn.Linear(2048, 5)
            self.classifier2 = nn.Linear(2048, self.num_classes)
        elif self.CAN_TS:
            self.dropout = nn.Dropout(0.3)
            self.branch_bam1 = CBAM(512 * block.expansion)
            self.branch_bam2 = CBAM(512 * block.expansion)
            self.classifier1 = nn.Linear(512 * block.expansion, 5)
            ...
            self.classifier1 = nn.Linear(300, 5)
            self.classifier2 = nn.Linear(300, self.num_classes)
